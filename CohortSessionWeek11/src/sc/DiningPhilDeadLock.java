...
class Philosopher extends Thread {
	...
	public void run() {
		...
		try {
			while (true) {
				// A fix to the deadlock
				// To ensure the last person picks up the fork in a different order
				if (index != 4){
					Thread.sleep(randomGenerator.nextInt(100)); //not sleeping but thinking
					System.out.println("Phil " + index + " finishes thinking.");
					left.pickup();
					// to ensure that everyone picks up their left spoon first before starting to pick up their right
					Thread.sleep(5000);
